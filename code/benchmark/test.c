...
int main(void) {
  ...
  printf("Addresses of the allocations (32-bit VAs): %" PRIx32 ", %" PRIx32
         ", %" PRIx32 "\n",
  ...
  for (i = 0; i < SIZE; i++) {
    for (j = 0; j < SIZE; j++) {
      address_a = add_offset32(a, (size_t)i * SIZE * sizeof(int) +
                                      (size_t)j * sizeof(int));
      address_b = add_offset32(b, (size_t)i * SIZE * sizeof(int) +
                                      (size_t)j * sizeof(int));
  ...
  for (i = 0; i < SIZE; i++) {
    for (j = 0; j < SIZE; j++) {
      address_c = add_offset32(c, (size_t)i * SIZE * sizeof(int) +
                                      (size_t)j * sizeof(int));
