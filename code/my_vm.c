...
void print_TLB_missrate(void) {
  ...
  fprintf(stderr, "Total lookups: %llu, Total misses: %llu\n", tlb_lookups,
          tlb_misses);
}
...
pte_t *translate(pde_t *pgdir, void *va) {
  ...
  /* Basically if we find the pte from the TLB, we don't need to make all the
   * address translations of the virtual address */
  if (pte_from_table) {
  ...
  uint32_t pt_pfn = pde >> PFN_SHIFT;
  /* This will basically give us the starting address of the page in the page
   * directory, from which we will find the exact page table entry */
  pte_t *pt = (pte_t *)(physical_memory + pt_pfn * PGSIZE);
...
void n_free(void *va, int size) {
  ...
  for (int i = 0; i < num_pages; i++) {
    ...
    if (pte == NULL || *pte == 0) {
      // printf("This virtual_address is either not mapped or is already
      // freed\n");
      continue;
    } else {
      uint32_t entry = *pte;
